...
router.post('/', (req, res) => {
  ...
  User.findOne({ email }).then((user) => {
    ...
    bcrypt.genSalt(10, (err, salt) => {
      bcrypt.hash(newUser.password, salt, (err, hash) => { // 2의 10승짜리 해시 값 생성
        if (err) throw err
...
router.post('/:userName/profile', auth, async (req, res) => {
  try {
    ...
    bcrypt.compare(previousPassword, result.password).then((isMatch) => {
      console.log(res, '/:userName/profile')
      if (!isMatch) {
        return res.status(400).json({ match_msg: '기존 비밀번호와 일치하지 않습니다.' })
      } else {
